...
Item {
    Component.onCompleted: {
        Qt.callLater(() => {
            gameBoard.startHosting(5500)
            console.log("networkManager is", gameBoard.networkManager)
        })
    }
    Connections {
    ...
    Rectangle{
        ...
        color: "black"
        Rectangle{
            implicitWidth: hostMultiplayerDialog.implicitWidth
            implicitHeight: hostMultiplayerDialog.implicitHeight
            anchors.centerIn: parent
            color: "black"
            Text {
                id: hostMultiplayerDialog
                text: qsTr("Waiting for another player to join your game...\nThe game will start shortly.")
                color: "white"
                anchors.centerIn: parent
